...
then
    ...
    /usr/bin/xeoma -core -service -log -noautoupdate -disableDownloads -startdelay 5
else
    echo "$(ts) Using archive cache"
    /usr/bin/xeoma -core -service -log -noautoupdate -disableDownloads -startdelay 5 -archivecache /archive-cache
fi
